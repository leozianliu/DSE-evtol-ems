...
class LoadsCalculator:
    ...
    def thrust_loads(self):
        ...
        torque_t[self.y_points <= self.gull_location] += self.engine_thrusts[1] * (
                    self.engine_positions_y[1] - self.gull_location) * math.sin(self.gull_angle)
    ...
    def engine_weight_loads(self):
        ...
        for pos, offset_z, offset_x, weight in zip(self.engine_positions_y, self.engine_offsets_z,
                                                   self.engine_offsets_x, self.engine_weights):
            mask = self.y_points <= pos
            if self.flight_mode == 'horizontal':
                ...
                normal_we[self.y_points > self.gull_location] += shear_we[
                                                                     self.y_points > self.gull_location] * math.tan(
                    self.gull_angle)
        ...
        if self.flight_mode == 'vertical':
            torque_we[self.y_points < self.gull_location] += self.engine_weights[1] * (
                        self.engine_positions_y[1] - self.gull_location) * math.sin(self.gull_angle)
    ...
    def aerodynamic_loads(self, lift, drag):
        ...
        shear_lift[self.y_points > self.gull_location] = shear_lift[self.y_points > self.gull_location] / math.cos(
            self.gull_angle)
...
shear_lift, shear_drag, moment_lift, moment_drag, normal_lift = calculator.aerodynamic_loads(
    lift=2.5 * aero.lift_gull_rh, drag=np.zeros(43, dtype=int))
...
if __name__ == "__main__":
    ...
    fig, axs = plt.subplots(2, 3, figsize=(12, 7))  # 2 rows, 3 columns
