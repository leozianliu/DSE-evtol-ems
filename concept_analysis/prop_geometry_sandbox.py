...
T = np.linspace(2000, 10000, 10)
v = np.linspace(20, 120, 10)
rpm = np.linspace(2000, 7000, 10)

n_blades = [5]

optim = Optimizer(2600, 1.2)
...
for T_val, v_val, rpm_val, n_val in itertools.product(T, v, rpm, n_blades):
    ...
    if 0 < optim.total_energy < best_energy and any(optim.power_arr_big) > 0 and 120 < optim.total_energy < 200:
